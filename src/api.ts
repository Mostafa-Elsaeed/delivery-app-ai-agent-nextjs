...
export const Bids = {
  list: async (orderId?: string) =>
    (await api.get("/bids", { params: { orderId } })).data,
  create: async (payload: any) => (await api.post("/bids", payload)).data,
  update: async (id: string, payload: any) =>
    (await api.patch(`/bids/${id}`, payload)).data,
};

// Wallets
export const Wallets = {
  get: async (user_id: string) => (await api.get(`/wallets/${user_id}`)).data,
  update: async (user_id: string, payload: any) =>
    (await api.patch(`/wallets/${user_id}`, payload)).data,
};
